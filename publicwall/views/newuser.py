...
ADD_POST = Permission.objects.get(name="Ability to add a post. ")
ADD_COMMENT = Permission.objects.get(name="Ability to make a comment.")
...
def newuser(request):
  ...
  if request.method == "POST":
    if request.POST['action'] == "validatecode":
      ...
      if checkValidCode(request.POST['code'], False):
        ...
        user.user_permissions.add(ADD_POST)
        user.user_permissions.add(ADD_COMMENT)
        user.save()
        checkValidCode(request.POST['code'], True)
