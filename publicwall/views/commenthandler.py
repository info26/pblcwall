  ...
  if request.method == "POST":
    ...
    try:
      for i in Post.objects.get(pk=request.POST['commentid']).comment_set.all():
        ...
        data.append({
          "user": User.objects.get(pk=i.user).username, 
          "text": i.comment_content, "date": dateformat, 
          'id':i.id
        })
    except:
      pass
  ...
  if request.method == "GET":
    ...
    if request.user.has_perm("publicwall.make-comment"):
      ...
      Post.objects.get(pk=request.POST['postid']).comment_set.create(
        comment_content=request.POST['commenttext'],
        date = tzz.now(),
      ...
      return JsonResponse({
        'ok': True, 
        'commentid': request.POST['postid'], 
        'user': request.user.username, 
        'date': loc.strftime("%b %d %Y %H:%M:%S")
      })
    else:
